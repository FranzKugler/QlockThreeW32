...
- `GET /languages` — `[{value, code, name, uiLocale}]`, in the order of the stored language numbers, so `value` is what `POST /configuration` wants. Static for a firmware; the shell asks once at load. Not part of `/currentState`: it is not a setting, it is what the firmware can do.
