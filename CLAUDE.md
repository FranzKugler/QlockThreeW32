...
- `GET /log?since=<seq>` — the log ring from that sequence number on, plus `oldest`, `more`, and the state block (`uptime`, `heap`, `heapMin`, `heapBlock`, `reset`). Not part of `/currentState`: none of it is a setting, and the debug tab polls it.
