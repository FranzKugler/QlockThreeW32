...
- **A cell is not always one character.** English wants O'CLOCK and Italian L'UNA, and on a real panel the letter and its apostrophe share one milled opening and one LED. The rule is that **U+2032 PRIME attaches to the character before it**, so `"TENSEO′CLOCK"` is twelve characters and eleven cells, and no separator is needed in the row string. `Languages::cells()` and `appendCell()` are the only two places that know it, and everything counting columns goes through them. PRIME rather than an ASCII apostrophe, which is a quote in half the files that touch this, and rather than U+2019, which is punctuation a font may kern into its neighbour.
- **Both were faked before, and both were wrong.** English wrote `Ò` for O' and Italian `Ľ` for L' — single characters that look roughly right and are not. They read wrongly on the wall, spelled wrongly in the log, and `selfCheck()` had been reporting three mismatches at every boot (`en at 9,5 says "OCLOCK" but the panel reads "ÒCLOCK"`, plus `È`/`E` and `ĽUNA`/`LUNA` in Italian) since the day the check was written. Nobody read the boot log — which is a fair description of what the debug tab exists for.
