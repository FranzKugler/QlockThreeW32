...
| [src/WebRoutes.cpp](src/WebRoutes.cpp) | `Web::` — every HTTP handler except `/ota/*` and `/portal/*`, the network-switch state machine, `serveFromFilesystem()` |
| [src/Portal.cpp](src/Portal.cpp) | `Portal::` — the setup portal: access point, DNS catch-all, the `/portal/*` endpoints, the connection attempt |
...
Two things make this a split rather than a rearrangement. **The modules take what they need as parameters instead of reading globals**: `Ota::poll(online, localHour)` gets told whether the network is up and what the local hour is, so nothing in it knows about `WiFi` or `Timezone`. And **`scheduleSettingsSave()` is the one named seam** both modules use to arm the deferred write, rather than each reaching into `timeToSaveToFLASH`.
...
[src/main .cpp](src/main%20.cpp) (note the literal space in the filename) is the entry point. `setup()` mounts LittleFS, loads `Settings`, tries the stored WiFi network and falls back to the setup portal (`Portal::begin()`, its own `QlockThreeW32` access point) if that fails, then starts NTP, mDNS, ArduinoOTA, RemoteDebug, the light sensor, and the `WebServer` on port 80. `loop()` re-syncs WiFi/NTP as needed, services the web server and OTA, and once per second rebuilds `matrix[16]` (the 16-row bitmask framebuffer) according to `mode` and pushes it to the LED driver. `mode` selects between normal time display, the same with a WiFi status pixel, dark, a seconds counter and an LED test pattern (the `STD_MODE_*` / `EXT_MODE_*` defines are in [src/DisplayModes.h](src/DisplayModes.h)).
...
**Portal mode only** (see "WiFi configuration" below — never registered alongside the endpoints above): `GET /portal/status`, `GET /portal/scan`, `POST /portal/connect`. `/wifi/scan` and `/portal/scan` answer in the same shape on purpose — one Svelte component, `NetworkList.svelte`, draws both.
...
**Two entry points, one build.** [vite.config.js](vite.config.js) builds `web/index.html` (`src/main.js` → `App.svelte`, the configuration SPA above) and `web/portal.html` (`src/portal.js` → [Portal.svelte](web/src/Portal.svelte), the setup portal) out of the same `web/` tree into `data/`. They share `app.css`, every locale file, and [web/src/lib/NetworkList.svelte](web/src/lib/NetworkList.svelte) — the scan-and-pick list `Wifi.svelte` and `Portal.svelte` both draw from, owning its own polling of whichever scan endpoint it is handed (`api.fetchWifiScan` or `api.fetchPortalScan`). `Portal.svelte` has no tab row (there is exactly one thing to do there) and picks its display language from the browser rather than following a clock — `preferBrowserLanguage()` in `i18n.svelte.js`, called only from `portal.js`, since a page served before any clock exists has nothing else to follow. See "WiFi configuration" below and [src/Portal.h](src/Portal.h) for why this page exists at all.
...
- **Connected** — the "WLAN" tab ([web/src/sections/Wifi.svelte](web/src/sections/Wifi.svelte)) shows status, scans, and switches networks through the `/wifi` endpoints above. The scan result is reduced to one entry per SSID, strongest first: a mesh or dual-band router answers once per radio, and the list is keyed by SSID — Svelte throws `each_key_duplicate` on a repeat, in production too, which took the whole list down rather than one row. `server.js` returns a duplicated SSID and a nameless one so the case shows up in development. The list itself is drawn by [web/src/lib/NetworkList.svelte](web/src/lib/NetworkList.svelte), which the setup portal below shares.
- **Not connected** — the SPA is unreachable, because it is served from LittleFS only once WiFi is up. [src/Portal.cpp](src/Portal.cpp) takes over in `setup()` instead: an open access point named `QlockThreeW32`, a `DNSServer` answering every name with its own address (the captive-portal trigger), and `/portal/status`, `/portal/scan`, `/portal/connect` on the same `WebServer` the SPA otherwise uses. **This is a second page out of the same Svelte build, not WiFiManager's own HTML with injected colours** — `portal.html`/`Portal.svelte` share `app.css`, every locale file and `NetworkList.svelte` with the SPA, so "same look and feel" is literally the same code rather than a stylesheet kept in step by hand. See [src/Portal.h](src/Portal.h) for the full rationale, including why nothing else is registered on the server while the portal is up (an open access point is not somewhere to expose the filesystem explorer or a firmware upload) and why it ends in a restart rather than a mode switch in place.

Switching networks (while already connected) is deliberately not a plain `WiFi.begin()`: a wrong password would leave the clock unreachable until someone power-cycles it and goes through the setup portal again. `POST /wifi` therefore only records the request and answers straight away (the response would never leave the old network otherwise), and a small state machine driven from `loop()` by `Web::poll()` does the rest: try the new credentials, and on timeout fall back to the previous SSID/PSK — captured via `WiFi.psk()` before the attempt — leaving an explanatory message in `wifiLastError` for the UI. The normal reconnect block in `loop()` is skipped while a switch is in flight so the two don't fight over the connection. `loop()` also returns early, before any of that, while `Portal::active()` — the portal's own DNS and connection attempt run instead, and nothing below (including that reconnect block) may touch the connection meanwhile.

The mock server implements the same endpoints for both paths — `/wifi`, `/wifi/scan` and, in a second mode started with `npm run mock -- portal`, `/portal/status`, `/portal/scan`, `/portal/connect` — including a simulated scan delay and fallback: connect with the password `wrong` to exercise the failure path, in either mode. A small gate near the top of `server.js` enforces the same "never both" split PORTAL_MODE gives the firmware: outside portal mode `/portal/*` 404s, and inside it everything else does.
...
- The page's own graphic is the real panel, not a mock-up: [scripts/flash_hero.py](scripts/flash_hero.py) draws the English grid from `Language_EN.cpp` with "IT IS FIVE PAST TWO" lit from the actual word positions, the same principle as the home screen icons below. It is the one light-mode-only graphic on the page — a still image cannot follow `prefers-color-scheme`, so it carries its own light background rather than a transparent one.
- **Two QR codes carry the two hand-offs after flashing**, generated offline by [scripts/flash_qr.py](scripts/flash_qr.py) — no service, no key, a QR code is just a fixed string turned into a bitmap. One is a WiFi-join code (`WIFI:T:nopass;S:QlockThreeW32;;`, the format iOS's and Android's own camera apps recognise) for the open access point the setup portal starts with no password (`Portal::begin()`, see "WiFi configuration" above); the other is plain text for `http://qlockthreew32.local`. Both bake in the clock's *default* hostname, which is exactly what an unconfigured clock still answers to at the point each code is meant to be scanned — and joining that first code now lands on the portal's own branded page rather than a generic one. Rendered plain black on white rather than the page's accent blue — colour in a QR code only narrows the contrast a camera has to resolve, and each of these gets scanned once, on a phone that for the first one is not yet on any network at all.
...
- **A point at the top of the range is left out of the fit.** It is *censored*: the slider had nothing above the maximum to offer, so "100 %" means "at least 100 %", and least squares reading it as an equality drags the bright end of the line down and flattens the slope. Kept and shown all the same — it is still what somebody said, it still stops a second point being taught at the same light, and a point that silently vanishes looks like a bug. `used` in `/luminance` says which points were in the fit; the screen draws the others hollow.
  - **The floor is deliberately not treated the same way.** The ceiling is what the hardware can do; the floor is a number the owner chose *on this very screen* as the dimmest they ever want, so a point sitting on it is a preference being met rather than a wish being cut off.
  - **Unless leaving them out leaves fewer than two points**, where a poor line beats no line.
  - **Moving the ceiling re-fits.** Which points count as censored depends on where it is, so `setRange()` calls `fit()` rather than only storing.
...
- `web/public/zones.json`, `web/public/*.png`, `docs/panel-preview.png`, `docs/qr-ap.png`, `docs/qr-clock.png`, `hardware/Qlock250mm/3dprint/panels.scad` — generated by the scripts in [scripts/](scripts/) and committed on purpose, so that neither a build nor a print needs Python. Edit the generator, not the output.
