<script>
  ...
  async function remove(row) {
    menu = null;
    if (!confirm(t.fsConfirmDelete(row.name))) return;
  ...
  /*
   * JSON arrives from both stores as one line - that is how the clock writes
   * its records and how a build tool writes version.json - and one line is not
   * something anybody can read, let alone correct. So it is laid out on the
   * way in.
   *
  ...
  /** What actually goes to the clock, given the compact switch. */
...
{#if store.canUpload || store.canMkdir}
  ...
  {#if creating !== null}
    <div class="actions">
      ...
      <input type="text" bind:value={creating} placeholder={t.fsFolderName} autofocus
...
<div class="tree" role="tree">
  ...
  {#each rows as row (row.path)}
    {#if row.truncated}
      <div class="row muted" style="--depth: {row.depth + 1}">{t.fsTruncated}</div>
    {:else}
...
<p class="hint gesture">{t.fsGesture}</p>
  ...
  <div class="menu" style="left: {menu.x}px; top: {menu.y}px" role="menu">
    ...
    {#if !menu.row.dir}
      <a role="menuitem" href={store.urlOf(menu.row.path)} download={menu.row.name}
         class:disabled={menu.row.protected} onclick={() => (menu = null)}>{t.fsDownload}</a>
      {#if menu.row.edit}
        <button type="button" role="menuitem" onclick={() => edit(menu.row)}>{t.fsEdit}</button>
      {:else}
        <button type="button" role="menuitem" disabled>
          {menu.row.protected ? t.fsProtected : t.fsBinary}
        </button>
      {/if}
    {/if}
    ...
    {#if !(menu.row.dir && store.key === 'nvs')}
      <button type="button" role="menuitem" class="danger"
              onclick={() => remove(menu.row)}>{t.fsDelete}</button>
    {/if}
  </div>
{/if}
  ...
  <div class="editor">
    <div class="editor-head">
      ...
      {#if editing.json}
        ...
        {#if !editorValid}<span class="invalid">{t.fsNotJson}</span>{/if}
      {/if}
    </div>
