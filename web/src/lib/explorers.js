 ...
 * the same way on purpose: namespaces read as folders, keys as files, and the
 * gestures are identical. One component draws both, so there is one place a
 * bug in the tree can live.
 *
 * The interface is deliberately narrow, and the differences it does admit are
 * the ones that would be lies to hide:
 *
...
/** The clock's LittleFS: the partition this page itself is served from. */
export function littleFs() {
  return {
    ...
    readText: (path) => api.fetchFile(path),
    urlOf: (path) => api.fileUrl(path),
    save: (path, content) => api.saveFile(path, content),
    remove: (path) => api.deleteEntry(path),
    mkdir: (path) => api.makeDirectory(path),
    upload: (path, file, onProgress) => api.uploadFile(path, file, onProgress)
  };
}
 ...
 * thing. A path here is `/namespace/key` and nothing else, so splitting it is
 * unambiguous.
 */
export function nvs() {
  ...
  return {
    ...
    async list(path) {
      // The root is where a refresh happens, because that is the one listing
      // the component reloads after every write.
    ...
    readText(path) {
      ...
      return api.readNvs(ns, stripSuffix(key));
    },

    urlOf(path) {
      const { ns, key } = split(path);
      return api.nvsUrl(ns, stripSuffix(key));
    },

    async save(path, content) {
      const { ns, key } = split(path);
      const answer = await api.saveNvs(ns, stripSuffix(key), content);
      snapshot = null;
      return answer;
    },
    ...
    async remove(path) {
      const { ns, key } = split(path);
      // A namespace is not a thing that can be deleted; it goes when its last
      // key does. Asking to delete a folder here is a mistake, not an action.
      if (key === undefined) throw new Error('nvsNamespaceDelete');
      const answer = await api.deleteNvs(ns, stripSuffix(key));
      snapshot = null;
      return answer;
    }
  };
